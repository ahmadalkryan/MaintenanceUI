...
import { useNavigate, useLocation } from 'react-router-dom';
import { addTicketTrace } from '../../api/ticketTraces';
import './AddTracePage.css';

const statusMap = {
    1: { label: 'Pending', class: 'badge--pending' },
    2: { label: 'Complete', class: 'badge--complete' },
    3: { label: 'Refund', class: 'badge--refund' }
};
...
export default function AddTracePage() {
    ...
    const [ticketIdInput, setTicketIdInput] = useState(ticket?.Id?.toString() || '');
    const [statusId, setStatusId] = useState('');
    const [note, setNote] = useState('');
    const [userId, setUserId] = useState('');
    ...
    const handleSubmit = async () => {
        const ticketId = +ticketIdInput;
        if (!ticketId || ticketId < 1) return setError('Ticket ID is required.');
        if (!statusId) return setError('Please select a status.');
        if (!userId || +userId < 1) return setError('User ID must be ≥ 1.');
        ...
        try {
            ...
            setError(err.message || 'Could not save trace.');
        } finally {
            setLoading(false);
        }
    };
